...
<template>
  <div v-if="isOpen" class="relative z-10" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    ...
    <div class="fixed inset-0 z-10 w-screen overflow-hidden">
      <div class="flex h-full items-center justify-center p-4 text-center sm:p-0" @click.self="$emit('close')">
        <div class="relative transform flex flex-col max-h-[calc(100vh-2rem)] overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-2xl">
          ...
          <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 shrink-0 border-t border-gray-100">
            ...
            <button @click="$emit('close')" type="button" class="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto">{{ $t('button.cancel') }}</button>
          </div>
        </div>
      </div>
    </div>
