...
const handleBulkDelete = async () => {
    ...
    if (confirm(`Are you sure you want to delete ${selectedIds.value.size} invoices?`)) {
        await invoicesStore.bulkDeleteInvoices(Array.from(selectedIds.value));
        selectedIds.value.clear();
    }
};
</script>
  ...
  <div>
    <div class="sticky top-0 z-10 bg-gray-50 py-4 px-8 sm:flex sm:items-center border-b border-gray-200/50 backdrop-blur-sm bg-gray-50/95">
      ...
      <div class="mt-4 sm:ml-16 sm:mt-0 sm:flex-none flex items-center gap-3">
        ...
        <router-link to="/admin/invoices/new" class="block rounded-md bg-indigo-600 px-3 py-2 text-center text-sm font-semibold text-white shadow-sm hover:bg-indigo-500">Create Invoice</router-link>
      </div>
    </div>
    <div class="p-8">
    <!-- Stats -->
    ...
    <div class="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-4">
        <div class="relative sm:col-span-2">
             <input v-model="searchQuery" type="text" class="h-9 block w-full rounded-md border-0 py-1.5 px-3 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6" placeholder="Search invoices...">
        </div>
        <div>
            ...
            <thead class="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th scope="col" class="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 sm:pl-3 cursor-pointer hover:text-gray-700" @click="toggleSort('invoiceNumber')">
                    Invoice # <span v-if="sortBy === 'invoiceNumber'">{{ sortOrder === 'asc' ? '↑' : '↓' }}</span>
                </th>
                 <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 cursor-pointer hover:text-gray-700" @click="toggleSort('invoiceDate')">
                    Date <span v-if="sortBy === 'invoiceDate'">{{ sortOrder === 'asc' ? '↑' : '↓' }}</span>
                 </th>
                <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 cursor-pointer hover:text-gray-700" @click="toggleSort('customerName')">
                    Customer <span v-if="sortBy === 'customerName'">{{ sortOrder === 'asc' ? '↑' : '↓' }}</span>
                </th>
                ...
                <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900 cursor-pointer hover:text-gray-700" @click="toggleSort('total')">
                    Total <span v-if="sortBy === 'total'">{{ sortOrder === 'asc' ? '↑' : '↓' }}</span>
                </th>
            ...
            <tbody class="divide-y divide-gray-200">
              ...
              <tr v-for="inv in invoices" :key="inv.id" :class="[selectedIds.has(inv.id) ? 'bg-gray-50' : 'hover:bg-gray-50/50', 'transition-colors']">
                ...
                <td class="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                  <router-link :to="`/admin/invoices/${inv.id}`" class="text-indigo-600 hover:text-indigo-900 mr-4">Edit</router-link>
                  <button @click="handleDelete(inv.id)" class="text-red-600 hover:text-red-900">Delete</button>
                </td>
              </tr>
            </tbody>
          </table>
